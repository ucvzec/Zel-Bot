...
process.on('exit',(exitCode)=>{
	logFile.log(`Exiting with ${exitCode}.`,"System");
...
process.on('SIGINT',()=>{
	logFile.log("SIGINT Recieved","System");
	//logs the bot out
...
client.on('message',(msg)=>{
	...
	if( !((!config.logBlacklistedUsers&&userBlacklist.includes(msg.author.id))|| (!config.logBlacklistedChannels&&channelBlacklist.includes(msg.channel.id)) ) ){
		//logs the connent of the message and the author
		logFile.log(`${msg.author.username}: ${msg.content}`,"Message");
...
function processMsg(msg){
	...
	let tempDone = false;
	for(let x=0,end=responses.length;x!=end;x++){
		...
		((schemeResults)=>{
			...
			if(schemeResults!==null){
				...
				//continue, because that means that this command identifier wasn't matched
			}
...
function addQuote(quote){
	...
	fs.writeFile("quoteList.json",JSON.stringify(quoteList,null,4),(error)=>{
		if(error){
			console.log(`Error writing to file, ${error.stack}`);
		}else{
			console.log(`Successfully wrote ${quote} to QuoteList.`);
		}
	});
}
...
rl.on('line',(input)=>{
	...
	if(input.startsWith("!eval")){
		...
		*/
		let validFileNames = ["responses","quoteList","config"];
		((fileName)=>{
			if(validFileNames.includes(fileName)){
				try{
					fs.readFile(input.split(" ")[2],(err,data)=>{
						((fileTypeCaps)=>{
							console.log(`${!err?`${fileTypeCaps} was successfully reloaded.`:`Error in reloading ${fileTypeCaps}`}`);
						})(fileName.slice(0,1).toUpperCase()+fileName.slice(1));
						eval(`${fileName}=JSON.parse(data)`);
					});
				}catch(err){
					console.log(err);
				}
...
/*
	...
	------------------------------------
	Basically these are used to process the responses token
